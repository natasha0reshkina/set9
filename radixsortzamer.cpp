...
void msd_radix_sort(std::vector<std::string> &arr, int low, int high, int d) {
    ...
    for (int b = 1; b < kNumBuckets; b++) {
        int start = low + prefix[b - 1];
        int end = low + prefix[b] - 1;
        if (start < end) {
...
void run_experiments(const std::string &array_type) {
    StringGenerator gen;
    std::string filename = "results_msd_radixsort_" + array_type + "_" + get_timestamp() + ".csv";
