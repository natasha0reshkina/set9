...
int lcp_compare(const std:: string &a, const std::string &b) {
...
void merge(std:: vector<std::string> &arr, int l, int m, int r) {
    ...
    while (i < left.size() && j < right.size()) {
        if (lcp_compare(left[i], right[j]) <= 0) {
            arr[k++] = left[i++];
        } else {
            arr[k++] = right[j++];
        }
...
void run_experiments(const std::string &array_type) {
    StringGenerator gen;
    std::string filename = "results_mergesort_" + array_type + "_" + get_timestamp() + ".csv";
